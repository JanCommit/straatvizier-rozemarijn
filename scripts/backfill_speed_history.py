"""Vul historische Telraam-snelheidshistogrammen aan in Supabase.

Gebruikt de segmentconfiguratie en historische API-data om de snelheidsverdeling
beschikbaar te maken voor de V50/V85/V95-berekeningen in StraatVizier."""
...
def month_has_speed_data(
    database_segment_id: int,
    start: datetime,
    end: datetime,
) -> bool:
    """
    Controleer of voor deze maand al snelheidsdata aanwezig zijn.
    """
...
def backfill_segment(segment_config: dict) -> dict:
    ...
    while current < END_DATE:
        ...
        if month_has_speed_data(
            database_segment_id,
            current,
            period_end,
        ):
            print(f"{label} ... snelheidsdata reeds aanwezig → overslaan")
            skipped += 1
            current = period_end
            continue
...
def main():
    ...
    print("=" * 70)
    print("STRAATVIZIER - HISTORISCHE SNELHEIDSBACKFILL")
    print("=" * 70)
