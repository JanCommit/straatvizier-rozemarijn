"""Valideer snelheidspercentielen over meerdere StraatVizier-segmenten.

Uitgebreidere variant van de percentielvalidatie om te controleren of de
histogrammethode ook over verschillende straten/segmenten consistent blijft."""
...
def main():
    ...
    # Neem per segment een aparte steekproef, zodat niet toevallig
    # slechts één straat/segment in de validatie terechtkomt.
    rows_by_segment = {}
    ...
    for segment_id, segment_rows in sorted(rows_by_segment.items()):
        segment_sample = random.sample(
            segment_rows,
            min(200, len(segment_rows)),
        )
        sample.extend(segment_sample)

        print(
            f"Segment {segment_id}: "
            f"{len(segment_sample)} records geselecteerd "
            f"uit {len(segment_rows)} bruikbare records"
        )
